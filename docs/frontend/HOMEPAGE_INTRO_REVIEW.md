...
Revised design after user feedback: original robotics/AI wallpaper remains visible through a transparent introduction; no new sky, stars or waving-hand emoji. Locally hosted IBM Plex Sans replaces Outfit/Syne on this page. Fine portrait ring and restrained rectangular buttons. The title still cycles through Wannes Fraj, AI Engineer and Edge AI Engineer. Keep actions together and visible before the portrait on mobile. Avoid invented experience claims, clipped long titles, hidden project navigation and uncontrolled motion.
...
- Title rotation works without a visible pause button, as requested by the user. Enabling reduced motion at runtime restores the static name. Static identity, CV access and project anchor work without JavaScript.
...
Local verification completed before the user explicitly authorized pushing the work. Mechanical detector flags the older shared gradient-text declaration; the homepage overrides it with a solid color. This existing rule is retained for other routes.
